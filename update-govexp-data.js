/**
 * update-govexp-data.js
 * ─────────────────────────────────────────────────────────────
 *  1. Reads MapDeptGroup.xlsx  (department mapping)
 *  2. Fetches ALL records from the Open Canada API
 *  3. Joins them together in memory
 *  4. Writes govexp-data.js  (pre-baked, CORS-free)
 *
 * Usage:  node update-govexp-data.js
 * ─────────────────────────────────────────────────────────────
 */
...
const OUTPUT_JS = 'govexp-data.js';
...
console.log('\n📊 T1 Tax Data — Gov\'t Expenses Data Builder');
...
function httpsGet(url) {
    return new Promise((resolve, reject) => {
        let body = '';
        https.get(url, { headers: { 'User-Agent': 'T1TaxDashboard/1.0' } }, res => {
...
(async () => {
    ...
    // Sort SOBJ categories alphabetically and remap
    ...
    const jsOutput = '/* Auto-generated by update-govexp-data.js — do not edit manually */\n' +
        'var GOVEXP_DATA = ' + json + ';\n';
